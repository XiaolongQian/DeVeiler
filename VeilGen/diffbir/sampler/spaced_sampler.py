...
class SpacedSampler(Sampler):
    ...
    def sample(
        ...
        self.make_schedule(steps) # 从1000步中选出steps个关键时间点
        self.to(device)
        if tiled: # 是否使用tile策略
        ...
        if x_T is None:
            x_T = torch.randn(x_size, device=device, dtype=torch.float32) # 纯噪声

        x = x_T
        ##
        # 假设 steps=50
        #self.timesteps = [0, 20, 40, 60, 80, 100, ..., 980, 999]  # 50个时间点
        #timesteps = np.flip(self.timesteps)  # [999, 980, 960, ..., 40, 20, 0]
        #total_steps = len(self.timesteps)    # 50
        ##
        ...
        bs = x_size[0]
        # 从timesteps中选出steps个关键时间点，然后进行采样
        for i, step in enumerate(iterator):
            model_t = torch.full((bs,), step, device=device, dtype=torch.long) # 当前时间步
            t = torch.full((bs,), total_steps - i - 1, device=device, dtype=torch.long) # 当前时间步的索引
