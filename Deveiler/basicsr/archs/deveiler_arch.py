...
class DDN(nn.Module):
    def __init__(self,
        ...
        codebook_scale = np.array(codebook_params)[:, 0]
        ...
        max_depth = int(np.log2(gt_resolution // codebook_scale[0]))
        ...
        self.lpe_enc_conv = nn.Conv2d(8, 256, 3, 1, 1)
        self.lpe_enc = nn.Sequential(
            ResidualBlockNoBN(num_feat=256),
            ResidualBlockNoBN(num_feat=256),
            ResidualBlockNoBN(num_feat=256)
        )
        self.lpe_conv = nn.ConvTranspose2d(256, 512, 4, 2, 1)
    ...
    def encode_and_decode(self, input,scale_map,shift_map):
        _,c,h,w = input.shape
        if h % 32 != 0 or w % 32 != 0:
            h_pad = (h // 32 + 1) * 32 - h
            w_pad = (w // 32 + 1) * 32 - w
            input = F.pad(input, (0, w_pad, 0, h_pad), mode='constant', value=0)
        img = input.split(3,1)[0]
        cac_feats = self.cac_encoder(img)
        cac_feats = cac_feats[::-1]


        x = cac_feats[0]

        lpe_feats = self.lpe_enc_conv(torch.cat([scale_map,shift_map],1))
        lpe_feats = self.lpe_enc(lpe_feats)

        lpe_feats = self.lpe_conv(lpe_feats)
        scale = lpe_feats[:,0:256,:,:]
        shift = lpe_feats[:,256:,:,:]
        x = x * (1 + scale) + shift

        residual_feature = self.cacdecoder(x)
        out_img_residual = self.residual_conv(residual_feature)
        out_img_residual = out_img_residual[:,:,:h,:w]


        return out_img_residual


    def forward(self, input,scale_map,shift_map):
        return self.encode_and_decode(input,scale_map,shift_map)

@ARCH_REGISTRY.register()
class DDN_frozen(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None,
...
class deveiler(nn.Module):
    def __init__(self,
        ...
        self.lpe_enc = MultiScaleEncoder(
                                in_channel,
                                max_depth,
                                gt_resolution,
                                channel_query_dict,
    ...
    def encode_and_decode(self, input):
        ...
        lpe_feats = self.lpe_enc(img)
        lpe_feats = lpe_feats[::-1]

        lpe_feats = lpe_feats[0]

        lpe_feats = self.lpe_conv(lpe_feats)
        scale_fea = 0.99 * lpe_feats[:,0:256,:,:]
        shift_fea = lpe_feats[:,256:,:,:]

        x = (x - shift_fea) / (1 + scale_fea)
        ...
        return out_img_residual, scale_fea, shift_fea
...
@ARCH_REGISTRY.register()
class deveiler_wo_vgcm(nn.Module):
    def __init__(self,
                 *,
                 in_channel=3,
                 codebook_params=None,
